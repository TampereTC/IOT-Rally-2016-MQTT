...
This robot is a Genuine IOT device with large set of sensors. Internet access is based on the ESP8266 Wifi-bridge chipset. Communication with the cloud server takes place via a bi-directional MQTT protocol. The robot is act as MQTT client and MQTT broker is located to the cloud server by publishing a sensor data to the cloud location and also subscribing control commands from MQTT broker.   
