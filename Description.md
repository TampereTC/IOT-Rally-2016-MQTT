...
This robot is a Genuine IOT device with large set of sensors. Arduino Mega2560 board is managing the sensor interface and logic of the Droid functions. Internet access is based on the ESP8266 Wifi-bridge chipset. Arduino is connected to ESP unit via serial port. Communication with the cloud server takes place via a bi-directional MQTT protocol. The robot is act as MQTT client and MQTT broker is located to the cloud server by publishing a sensor data to the cloud location and also subscribing control commands from MQTT broker. Controlling the Droid is happening via the other MQTT client which is connected to the same mqtt broker as the device. Typical MQTT clients are like web browsers or mobile apps.    
